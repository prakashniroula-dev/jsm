...
function node(type, props, componentName) {
  return {nodeType: type, props, __node: true, componentName};
}

export function Jsm(componentFn, componentName) {
  return function(...args) {
    const props = JsmProps(args);
    return node(componentFn, props, componentName || componentFn.name || 'JsmComponent');
  }
}
...
export function JsmDom(tag, ...args) {
  ...
  return node(tag, props, tag[0].toUpperCase() + tag.slice(1));
}